...
from flow import NeuralSplineFlow, NormalizingFlow
...
def main(teacher_score_func, img_saver, true_samples, opt):
    ...
    if opt.flow_type == 'NSF':
        flow = NeuralSplineFlow(dim=2, flow_length=opt.flow_length).to(opt.device)
    elif opt.flow_type == 'PLF':
        flow = NormalizingFlow(dim=2, flow_length=opt.flow_length).to(opt.device)
    else:
        raise NotImplementedError

    decoder_optim = optim.Adam(decoder.parameters(), lr=opt.lvm_lr)
    encoder_optim = optim.Adam(list(diffusion_encoder.parameters())+list(flow.parameters()), lr=opt.lvm_lr)
    ...
    with open(f"{opt.proj_path}/timing_log.txt", "a") as f:
        for it in tqdm(range(1, opt.max_iter + 1)):
            ...
            z_mu_flow, log_dets = flow(z.detach(),xt.detach(), sigma)
            logqz_xt = Normal(z_mu, z_std).log_prob(z_mu_flow).sum(-1)
            ...
            loss = -(weight * (logqz_xt + log_dets + (mean_diff * x0).sum(-1))).mean()
            loss.backward()
            ...
            torch.nn.utils.clip_grad_norm_(flow.parameters(), max_norm=opt.grad_norm_clip)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--flow_length", type=int, default=4)
    parser.add_argument("--lvm_lr", type=float, default=1e-4)
    ...
    if args.name is None:
        ...
        args.name = f"{args.flow_type}_smin{args.sigma_min}_{args.power}_smax{args.sigma_max}_{weight_str}_{score_str}"
    ...
    if opt.load is not None:
        opt.load_path=f"""./NSF/{opt.load}.pth"""
    ...
    if opt.target=="mog9":
        ...
        if opt.true_score:
            ...
            from utils.edm_utils import edm_sample_2D
            ...
            print('----sampling from pre-train EDM diffusion.')
            edm_samples = edm_sample_2D(teacher_score_func,num_samples=10000,num_steps=100,device=opt.device)
            log_prob = eval_log_prob(samples=edm_samples.to(opt.device),
                                     mean_scale=torch.ones([10000,1],device=opt.device),
                                     std=torch.ones([10000,1],device=opt.device)*torch.nn.functional.softplus(torch.tensor(1.0,device=opt.device)))
            mean_mmd = eval_mean_mmd(true_samples.to(opt.device), edm_samples.to(opt.device), opt.sigma_list)
            img_saver(edm_samples,save_name=f'edm samples,log_prob={log_prob},mmd={mean_mmd}')
