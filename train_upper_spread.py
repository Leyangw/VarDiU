...
def main(target, img_saver, opt):
    ...
    for it in tqdm(range(opt.max_iter)):
        ...
        logpx= target.log_prob(x).sum()
        score_logpx=torch.autograd.grad(logpx, x, create_graph=False)[0].detach()
        loss = (logpxz + logpz - logqz_x - (score_logpx*x).sum(-1)).mean()
        # print(loss.item())
        loss.backward()
        wandb.log({
            'loss':loss.item(),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--x_std", type=float, default=2.0) 
    ...
    if opt.target=="mog9":
        ...
        target = GMM(dim=2, n_mixes=40,loc_scaling=40, log_var_scaling=1,spread_std=opt.x_std,device=opt.device)
        target.to(opt.device)
        img_saver=lambda x,save_name: plot_MoG40(
            log_prob_function=GMM(dim=2, n_mixes=40,loc_scaling=40, log_var_scaling=1,device="cpu", spread_std=0.0).log_prob,
            samples=x, 
            save_path=opt.proj_path+'/plot',
            save_name=save_name,
            title=None
