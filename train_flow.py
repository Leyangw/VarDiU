...
def main(teacher_score_func, img_saver, true_samples, opt):
    ...
    encoder_optim = optim.Adam(list(diffusion_encoder.parameters()), lr=opt.lvm_lr)
    flow_optim = optim.Adam(list(flow.parameters()), lr=opt.lvm_lr)
    ...
    with open(f"{opt.proj_path}/timing_log.txt", "a") as f:
        for it in tqdm(range(1, opt.max_iter + 1)):
            ...
            z_mu_flow, log_dets = flow(z.detach(), sigma)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--flow_length", type=int, default=16)
    ...
    if args.name is None:
        ...
        args.name = f"{args.flow_type}_smin{args.sigma_min}_smax{args.sigma_max}_{weight_str}_{score_str}"
    ...
    if opt.target=="mog9":
        ...
        if opt.true_score:
            ...
            log_prob = eval_log_prob(samples=edm_samples.to(opt.device),
                                     mean_scale=torch.ones([10000,1],device=opt.device),
                                     std=torch.ones([10000,1],device=opt.device)*torch.nn.functional.softplus(torch.tensor(1.0,device=opt.device)))
            img_saver(edm_samples,save_name=f'edm samples,log_prob={log_prob}')
